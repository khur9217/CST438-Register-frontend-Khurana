...
class AddStudent extends Component {
  ...
  // Save student and close modal form
    handleAdd = () => {
       this.props.addStudent({name: this.state.name, email: this.state.email, status: this.state.status, status_code: this.state.status_code});
       this.handleClose();
    }
    ...
    render()  { 
      return (
          <div>
            <Button id="addstud" variant="outlined" color="primary" style={{margin: 10}} onClick={this.handleClickOpen}>
              Add Student
            </Button>
